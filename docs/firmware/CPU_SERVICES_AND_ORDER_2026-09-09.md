...
2026-09-25補足：転送順だけでは画面配置は決まらない。実ROMのLCD DDRAMは
上段`00 02 04 06`／下段`01 03 05 07`で、字形は5パートごとの上下ペアとなる。
上段0〜3／下段4〜7というnative側の誤配置を修正した。
`tools/lcd-meter-oracle`でH8受信からLCD_Renderまで実行し、451パターンと
ユーザー提供RCPの109画像について各55,296セル内ピクセルの一致を確認した。
試験範囲と実行手順は同ディレクトリのREADMEを参照。
