...
inline unsigned ModulationWork(const sc55::ModulationBlock& block,uint16_t ticks,
    const std::array<uint16_t,256>& rates,const sc55::LfoWaveformTables& tables,uint16_t randomWord,unsigned* randomLatchInstructions=nullptr)
{
    if(randomLatchInstructions) *randomLatchInstructions=0;
    unsigned count=3;
    bool delayed=false;
    ...
    // 3cbf/3ce9 reads E034 and latches the shared PCM value. The
    // following E03A word read consumes that latch, not a fresh random value.
    if(sample && randomLatchInstructions)
        *randomLatchInstructions=count+7+(block.waveform==4 ? 0 : block.waveform==5 ? 2 : 1);
