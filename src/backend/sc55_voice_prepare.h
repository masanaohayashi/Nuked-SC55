...
// One/two distinct normal restarted owners only; no mono overwrite semantics.
    ...
    std::array<uint8_t,24>& activity) noexcept
{
    ...
    for (unsigned partial = 0; partial < 2; ++partial)
    {
        ...
        if (sample.slot >= 24 || (slots&(1u<<sample.slot)) || (sample.sample.sampleId&0x8000)
            || installed.input.tone != selection.tone || installed.input.partial != partial
            || installed.input.sample != sample.sample.sampleId || !(installed.flags&128)
            || !selection.partials.partials[partial] || input.sourceKey >= 128) return std::nullopt;
...
// 5542..559e /55de..560e normal restarted preparation in dispatcher order.
    ...
    const PitchConversion& conversion,const LfoWaveformTables& waves,Read&& read,Write&& write)
{
    ...
    PreparedNormalVoiceBatch result;
    ...
    for (unsigned i = 0; i < result.count; ++i)
    {
        ...
        if (entry.slot >= 24 || input.partial >= 2 || input.part >= 16 || input.originalKey >= 128
            || input.sample != request.sample.sampleId || !(installed.flags&128) || entry.lifecycle.fieldCAF4 != 0
        ...
        const auto samplePcm = PreparePartialSamplePcm(request.sample,*data.samples(),entry.slot,
            request.unoffsetStart,request.historyNibble);
        const auto amplitude = PreparePartialAmplitude(*patch,input.partial,request.sample,*data.samples(),
        ...
        if (!samplePcm || !amplitude) return std::nullopt;
        result.voices[i].emplace(PreparedNormalVoice{VoiceControlState{*amplitude},request.controls,request.firstControls,{},{}});
    ...
    std::array<SecondEnvelopeSetup,2> secondSetup;
    ...
    for (unsigned i = 0; i < result.count; ++i)
    {
        ...
        first[slot] = {}; second[slot] = {}; secondSources[slot] = 24;
    ...
    if (InitializeFirstVoiceModulation(firstSlot,first,result.voices[0]->firstControls,modulation.timing,
    ...
    for (unsigned i = 0; i < result.count; ++i)
    {
        ...
        if (!voice.output.spatial.initialize(controls.spatial,*data.pan(),read,write)) return std::nullopt;
        ...
        if (!setup.prepare(partial,input.originalKey,request.secondary,voice.release.second,voice.second,
            ...
            *data.times(),*data.secondEnvelope())) return std::nullopt;
