...
struct BasicVoiceLinks
{
    static_assert(Capacity > 0 && Capacity <= 128);
    static constexpr unsigned count = Capacity;
    static constexpr uint8_t absent = 0xff;
    std::array<uint8_t,count> first, second;
    BasicVoiceLinks() noexcept { first.fill(absent); second.fill(absent); }
...
template<unsigned Capacity>
struct BasicVoiceGroupLinks
{
    std::array<uint8_t,Capacity> next, previous, head, tail;
    BasicVoiceGroupLinks() noexcept
    { next.fill(0xff); previous.fill(0xff); head.fill(0xff); tail.fill(0xff); }

    bool detach(unsigned voice, unsigned group, BasicVoiceLinks<Capacity>& pcmLinks) noexcept
    {
        if (voice >= Capacity || group >= Capacity) return false;
        ...
        if (successor < 128)
        {
            if (successor >= Capacity) return false;
        ...
        else
        {
            if (predecessor < 128)
            {
                if (predecessor >= Capacity) return false;
                previous[voice] = 0xff;
