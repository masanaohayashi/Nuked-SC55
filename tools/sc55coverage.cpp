...
struct Range { uint8_t cp; uint16_t lo, hi; const char* what; uint64_t hits; };
static Range R[] = {
    { 0, 0x309b, 0x3188, "音量合成 + 修飾", 0 },
    { 0, 0x35c9, 0x36a9, "エンベロープ生成 + 符号化", 0 },
    { 0, 0x36db, 0x3735, "TVA", 0 },
    { 0, 0x37b1, 0x37fc, "エフェクト送り", 0 },
    { 0, 0x3b26, 0x3d00, "LFO + 遅延／立ち上がり", 0 },
    { 0, 0x3e30, 0x3e6d, "フィルタモード", 0 },
    { 0, 0x41e4, 0x4251, "レゾナンス", 0 },
    { 0, 0x473c, 0x47fc, "カットオフ", 0 },
    { 0, 0x488f, 0x48b4, "ピッチ基準", 0 },
    { 0, 0x51b0, 0x5279, "ピッチ変換", 0 },
    { 0, 0x5300, 0x5368, "ピッチ書き出し", 0 },
    { 0, 0x5cee, 0x5ec9, "コントローラ目盛り", 0 },
    // 表示系（音声には効かない）
    { 4, 0x44aa, 0x4545, "レベルメーター計算", 0 },
...
int main(int argc,char**argv){
  ...
  tr=false;
  uint64_t sum=0, disp=0;
  std::printf("実行命令 %llu\n", (unsigned long long)total);
  for(auto&r:R){ if(r.cp==0) sum+=r.hits; else disp+=r.hits;
