...
namespace {
inline void verifyNativeFilterConnections(mcu_t& cpu)
{
    constexpr uint16_t entries[]{0x46c3,0x46c6,0x46ca,0x46cd,0x46d0,0x46d3,0x46d7,0x46da,
        0x4448,0x47ee,0x47f4,0x47fa,0x4856};
    uint32_t random = 55;
    ...
    for (auto entry : entries) for (unsigned variant = 0; variant < 384; ++variant) {
        ...
        if (!mcu_native::TryStepFilterConnections(cpu) || cpu.native_debt)
            throw std::runtime_error("Filter connection rejected");
        ...
        if (cpu.pc != nextPc || cpu.sr != nextSr
            ...
            std::fprintf(stderr,"Filter connection mismatch at %04x variant %u\n",entry,variant);
            throw std::runtime_error("Filter connection differs from H8");
        }
        ++cases;
    }
    std::printf("Native filter connections: %u instruction boundaries matched\n",cases);
}
}
