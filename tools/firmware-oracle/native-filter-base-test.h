...
namespace {
inline void verifyNativeFilterBase(mcu_t& cpu)
{
    uint32_t random = 55;
    ...
    auto check = [&](unsigned kind, uint8_t control, uint8_t base, uint16_t scale1, uint16_t scale2, uint16_t level) {
        ...
        cpu.r[0] = uint16_t(voice); MCU_Write16(cpu,voice+46,0x9600); MCU_Write(cpu,0x9612,control);
        MCU_Write(cpu,voice+163,base); MCU_Write(cpu,voice+162,kind ? 4 : 0);
        MCU_Write16(cpu,voice-2,uint16_t(cases%24)); cpu.r[5] = scale1;
        MCU_Write16(cpu,voice+136,scale2); (void)level;
        ...
        if (!mcu_native::TryAdjustFilterBase(cpu)) throw std::runtime_error("Filter base rejected");
        ...
        if (steps != count || cpu.pc != pc || cpu.sr != sr
            ...
            std::fprintf(stderr,"Filter base kind=%u control=%u base=%u scales=%u/%u count=%u/%u SR=%04x/%04x\n",
                         kind,control,base,scale1,scale2,steps,count,cpu.sr,sr);
            throw std::runtime_error("Filter base differs from H8");
        }
        ++cases;
    };
    ...
    std::printf("Filter base: %u register/SR/SRAM/count cases matched\n",cases);
}
}
