...
namespace {
inline void verifyNativeFilterSetup(mcu_t& cpu)
{
    constexpr uint16_t entries[]{0x43d1,0x43d4,0x43d8,0x43db,0x43de,0x43e2,0x43e5,0x43e8,0x43ec,0x43ef,0x43f2,0x43f6,0x43f9,0x43fc,0x4400,0x4403,0x4406,0x4409,0x440c,0x440f,0x4412,0x4415,0x4417,0x441a,0x441e,0x4422,0x4428,0x442b,0x442d,0x4431,0x4435,0x4438,0x443d,0x4440};
    uint32_t random = 55;
    ...
    for (auto entry : entries) for (unsigned variant = 0; variant < 384; ++variant) {
        ...
        if (!mcu_native::TryStepFilterSetup(cpu) || cpu.native_debt)
            throw std::runtime_error("Second filter setup rejected");
        ...
        if (cpu.pc != nextPc || cpu.sr != nextSr || cpu.ep != nextEp || cpu.dp != nextDp || cpu.ex_ignore != nextIgnore
            ...
            std::fprintf(stderr,"Second filter setup mismatch at %04x variant %u\n",entry,variant);
            throw std::runtime_error("Second filter setup differs from H8");
        }
        ++cases;
    }
    std::printf("Native filter setup: %u instruction boundaries matched\n",cases);
}
}
