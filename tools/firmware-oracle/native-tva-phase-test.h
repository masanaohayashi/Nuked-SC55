...
namespace {
inline void verifyNativeTvaPhase(mcu_t& cpu)
{
    uint32_t random = 55;
    ...
    auto check = [&](uint16_t duration, uint16_t elapsed, uint16_t residual, uint16_t phase) {
        ...
        cpu.pc = 0x358e; cpu.sr = uint16_t(0x0700|(cases&15)); cpu.native_debt = 0;
        for (auto& reg : cpu.r) reg = next();
        const unsigned voice = 0xacde + (cases%24)*0x12a;
        cpu.r[0] = uint16_t(voice); cpu.r[6] = duration;
        MCU_Write16(cpu,0xac5a,elapsed); MCU_Write16(cpu,voice+18,residual);
        MCU_Write16(cpu,voice+8,phase); MCU_Write(cpu,voice+97,uint8_t(next()));
        ...
        if (!mcu_native::TryAdvanceTvaPhase(cpu)) throw std::runtime_error("TVA phase advance rejected");
        ...
        cpu.pc = 0x358e; cpu.sr = initialSr; cpu.native_debt = 0;
        ...
        if (steps != count || cpu.pc != pc || cpu.sr != sr
            ...
            std::fprintf(stderr,"TVA phase advance %u/%u/%u/%u count=%u/%u SR=%04x/%04x\n",
                         duration,elapsed,residual,phase,steps,count,cpu.sr,sr);
            throw std::runtime_error("TVA phase advance differs from H8");
        }
        ++cases;
    };
    ...
    std::printf("TVA phase advance: %u register/SR/SRAM/count cases matched\n",cases);
}
}
