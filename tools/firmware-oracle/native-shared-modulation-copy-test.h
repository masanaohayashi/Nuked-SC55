...
namespace {
inline void verifyNativeSharedModulationCopy(mcu_t& cpu)
{
    constexpr uint16_t entries[]{0x3aae,0x3ab1,0x3ab4,0x3ab7,0x3aba,0x3abd,0x3ac0,0x3ac3,0x3ac6,0x3ac9,0x3acc,0x3acf,0x3ad2,0x3ad5,0x3ad8,0x3adb,0x3ade,0x3ae1,0x3ae4,0x3ae7,0x3aea};
    uint32_t random = 55;
    ...
    for (auto entry : entries) for (unsigned variant = 0; variant < 384; ++variant) {
        ...
        if (!mcu_native::TryStepSharedModulationCopy(cpu) || cpu.native_debt)
            throw std::runtime_error("Shared modulation copy rejected");
        ...
        if (cpu.pc != nextPc || cpu.sr != nextSr
            ...
            std::fprintf(stderr,"Shared modulation copy mismatch at %04x variant %u\n",entry,variant);
            throw std::runtime_error("Shared modulation copy differs from H8");
        }
        ++cases;
    }
    std::printf("Native shared modulation copy: %u instruction boundaries matched\n",cases);
}
}
