...
namespace {
inline void verifyNativeFilterResonance(mcu_t& cpu)
{
    uint32_t random = 55;
    ...
    auto check = [&](unsigned kind, uint8_t control, uint8_t base, uint16_t scale1, uint16_t scale2, uint16_t level) {
        ...
        cpu.r[0] = uint16_t(voice); MCU_Write16(cpu,voice+46,0x9600); MCU_Write(cpu,0x9613,control);
        MCU_Write(cpu,voice+103,base); MCU_Write(cpu,voice+105,uint8_t(scale1));
        MCU_Write16(cpu,voice-2,uint16_t(cases%24)); MCU_Write(cpu,voice+104,uint8_t(scale2));
        MCU_Write16(cpu,voice+36,level); (void)kind;
        ...
        if (!mcu_native::TryAdjustFilterResonance(cpu)) throw std::runtime_error("Filter resonance rejected");
        ...
        if (steps != count || cpu.pc != pc || cpu.sr != sr
            ...
            std::fprintf(stderr,"Filter resonance kind=%u control=%u base=%u scales=%u/%u count=%u/%u SR=%04x/%04x\n",
                         kind,control,base,scale1,scale2,steps,count,cpu.sr,sr);
            throw std::runtime_error("Filter resonance differs from H8");
        }
        ++cases;
    };
    ...
    for (unsigned kind = 0; kind < 2; ++kind) {
        for (unsigned control = 0; control < 256; ++control)
            for (unsigned base : {0u,1u,63u,64u,126u,127u})
                for (auto scale : edges) check(kind,uint8_t(control),uint8_t(base),scale,uint16_t(65535-scale),scale);
        for (unsigned i = 0; i < 1024; ++i) {
            const auto a = next(), b = next(), c = next(), d = next();
            check(kind,uint8_t(a&127),uint8_t(b&127),c,d,i%7 ? 1 : 0);
        }
    }
    std::printf("Filter resonance: %u register/SR/SRAM/count cases matched\n",cases);
}
}
