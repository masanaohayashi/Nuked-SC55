...
namespace {
inline void verifyNativeTvaStage(mcu_t& cpu)
{
    uint32_t random = 55;
    auto next = [&] { random = random*1664525u+1013904223u; return uint16_t(random>>16); };
    unsigned cases = 0;
    auto check = [&](uint16_t stage, uint16_t phase) {
        cpu.native_v121_enabled = true; cpu.cp = cpu.dp = cpu.ep = 0;
        cpu.pc = 0x33f4; cpu.sr = uint16_t(0x0700|(cases&15)); cpu.native_debt = 0;
        for (auto& reg : cpu.r) reg = next();
        const unsigned voice = 0xacde + (cases%24)*0x12a;
        cpu.r[0] = uint16_t(voice); MCU_Write16(cpu,voice,stage);
        MCU_Write16(cpu,voice+8,phase); MCU_Write16(cpu,voice-2,uint16_t(cases%24));
        for (unsigned i = 0; i < 3; ++i) {
            MCU_Write(cpu,voice+80+i,uint8_t(next())); MCU_Write(cpu,voice-7+i,uint8_t(next()));
            MCU_Write(cpu,voice+98+i,uint8_t(next()));
        }
        MCU_Write(cpu,voice+97,uint8_t(next()));
        ...
        if (!mcu_native::TryDispatchTvaStage(cpu)) throw std::runtime_error("TVA stage dispatch rejected");
        ...
        cpu.pc = 0x33f4; cpu.sr = initialSr; cpu.native_debt = 0;
        ...
        if (steps != count || cpu.pc != pc || cpu.sr != sr
            ...
            std::fprintf(stderr,"TVA stage dispatch stage=%u phase=%u count=%u/%u SR=%04x/%04x\n",
                         stage,phase,steps,count,cpu.sr,sr);
            throw std::runtime_error("TVA stage dispatch differs from H8");
        }
        ++cases;
    };
    for (unsigned repeat = 0; repeat < 128; ++repeat)
        for (uint16_t stage = 0; stage <= 22; stage += 2)
            for (auto phase : {uint16_t(0),uint16_t(1),uint16_t(0xfffe),uint16_t(0xffff)}) {
                if (stage > 12 && phase != 0xffff) continue;
                check(stage,phase);
            }
    std::printf("TVA stage dispatch: %u register/SR/SRAM/count cases matched\n",cases);
}
}
