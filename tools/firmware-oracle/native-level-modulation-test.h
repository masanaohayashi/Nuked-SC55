...
namespace {
inline void verifyNativeLevelModulation(mcu_t& cpu)
{
    uint32_t random = 55;
    ...
    auto check = [&](uint16_t depth1, uint16_t depth2, uint16_t base, uint16_t wave) {
        ...
        cpu.pc = 0x312b; cpu.sr = uint16_t(0x0700|(cases&15)); cpu.native_debt = 0;
        for (auto& reg : cpu.r) reg = next();
        const unsigned voice = 0xacde + (cases%24)*0x12a;
        cpu.r[0] = uint16_t(voice); cpu.r[2] = depth1; cpu.r[3] = depth2; cpu.r[4] = base; cpu.r[6] = wave;
        ...
        if (!mcu_native::TryModulateLevel(cpu)) throw std::runtime_error("Level modulation rejected");
        ...
        cpu.pc = 0x312b; cpu.sr = initialSr; cpu.native_debt = 0;
        ...
        if (steps != count || cpu.pc != pc || cpu.sr != sr
            ...
            std::fprintf(stderr,"Level modulation %u/%u/%u/%u count=%u/%u SR=%04x/%04x\n",
                         depth1,depth2,base,wave,steps,count,cpu.sr,sr);
            throw std::runtime_error("Level modulation differs from H8");
        }
        ++cases;
    };
    ...
    std::printf("Level modulation: %u register/SR/SRAM/count cases matched\n",cases);
}
}
