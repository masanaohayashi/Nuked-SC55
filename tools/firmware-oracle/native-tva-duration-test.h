...
namespace {
inline void verifyNativeTvaDuration(mcu_t& cpu)
{
    uint32_t random = 55;
    ...
    auto check = [&](unsigned kind, uint8_t control, uint8_t base, uint16_t scale1, uint16_t scale2, uint16_t level) {
        ...
        const uint16_t entry = kind == 0 ? 0x348c : kind == 1 ? 0x34e2 : 0x3536;
        ...
        MCU_Write(cpu,voice+79,base); MCU_Write16(cpu,voice+28,level);
        MCU_Write16(cpu,voice-(kind == 2 ? 32 : 34),scale1);
        MCU_Write16(cpu,voice-(kind == 0 ? 30 : 28),scale2);
        ...
        if (!mcu_native::TryComputeTvaDuration(cpu)) throw std::runtime_error("TVA duration rejected");
        ...
        if (steps != count || cpu.pc != pc || cpu.sr != sr
            ...
            std::fprintf(stderr,"TVA duration kind=%u control=%u base=%u scales=%u/%u count=%u/%u SR=%04x/%04x\n",
                         kind,control,base,scale1,scale2,steps,count,cpu.sr,sr);
            throw std::runtime_error("TVA duration differs from H8");
        }
        ++cases;
    };
    ...
    std::printf("TVA duration: %u register/SR/SRAM/count cases matched\n",cases);
}
}
